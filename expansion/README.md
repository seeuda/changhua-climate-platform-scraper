...
### 注意：data-engine 有兩份 changhua_towns.json，只有一份是對的

`changhua-climate-data-engine-v1` 的**根目錄**與 **`data/`** 各有一份
同名的 `changhua_towns.json`，內容不同：

| 檔案 | 大小 | bbox 西界 | 是否被載入 | 位置正確性 |
|------|------|----------|-----------|-----------|
| `data/changhua_towns.json` | 1.85 MB | 120.2204 | ✅ `app.js:571` | **正確** |
| `changhua_towns.json`（根目錄） | 4.78 MB | 120.2504 | ❌ 無人載入 | 東偏約 3,051 公尺 |

以四組互相獨立的點位驗證 `data/` 那份，全部 100% 落在登記鄉鎮內
（日照 88/88、環保設施 52/52、身障福利 11/11、本專案 300/300），
零位移。**前端載入的是正確的那份，線上系統沒有問題。**

根目錄那份是位置偏移的殘留檔，未被任何程式引用，但檔名相同且體積更大，
容易誤取（本專案就曾誤用而得出「圖資有問題」的錯誤結論）。建議刪除，
或改名為 `changhua_towns.legacy.json` 以免再被誤用；`README.md` 的專案
結構圖也把它列在 `data/` 底下，與實際位置不符。

本專案的 `staging/changhua_towns.json` 取自 `data/` 那份。
