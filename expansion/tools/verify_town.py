...
⚠️ 鄉鎮界檔案務必取自 data-engine 的 **data/changhua_towns.json**
（app.js 實際載入的那份，位置正確，實測四組點位皆 100% 相符）。該 repo
根目錄另有一份同名但東偏約 3 公里的殘留檔，取錯會產生大量假警報。

--calibrate 會估算圖資與點位之間的系統性位移；位移顯著時本工具拒絕給
逐筆判定，正是為了擋掉這種取錯檔案的情況。
